...
async function createCredential(headers, signal) {
    const optionsResponse = await fetchWithErrorHandling('/passkey/creation-options', {
        method: 'POST',
        headers,
        signal,
    });
...
async function requestCredential(email, mediation, headers, signal) {
    const optionsResponse = await fetchWithErrorHandling(`/passkey/request-options?username=${email}`, {
        method: 'POST',
        headers,
        signal,
    });
